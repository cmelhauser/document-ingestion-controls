...
## Optional client-configured business platform

`business_data_platform_v1` YAML is the secret-free tenant deployment contract
for one exact approved retrieval snapshot. Its configuration SHA-256 binds the
analytics model and private change journal. The complete closed field contract,
limits, examples, and deployment rules are in
[Business Data Platform](business-data-platform.md).

Typed analytics emits `governed_analytics_semantic_model_v1` and
`governed_analytics_result_v1`. Each result retains the configuration and source
snapshot identities, exact plan, effective dimensions, row/group coverage,
exceptions, pagination, optional totals, rows, and `result_sha256`. Invalid
metric values are explicit exceptions; join fan-out and arbitrary SQL are
refused. Remote analytical CSV/XLSX artifacts reuse
`crm_downloadable_export_v1` with `subject.kind: analytics`, owner binding,
short expiry, row/source/file checksums, and formula-injection defenses.

The private `business_record_change_journal_v1` is tenant/config-bound and
append-only. It retains proposal, signed authorization, target application, and
reconciliation events with per-event integrity and idempotency. The artifact
sequence is `business_record_change_v1`,
`business_record_change_preview_v1`,
`business_record_change_authorization_v1`,
`business_record_change_application_v1`, and
`business_record_change_reconciliation_v1`; lifecycle inserts return
`business_record_change_event_receipt_v1`. A proposal is never authorization,
the target application never mutates the served snapshot, and reconciliation
always requires a new approved snapshot before reads change.
...
| 17B | `canonical_export.py` | Intake manifest, consensus, arithmetic, the final-review queue, and optionally the classification consensus | Canonical export envelope plus a retained exclusion artifact; a document enters only with intake provenance, a review-clear status, non-blocking arithmetic, and no open review item, and an agreed field with no canonical column is withheld rather than mapped. A document the extraction consensus left `unknown` takes the type the classification consensus accepted; a type extraction established is kept. It approves nothing. |
...
| A vote settles the findings on the fields it accepted | `client_review/queue.py` `vote_settlements`, `consolidate(paths, accepted, governed, settled)`, `--settled-by-vote VOTE ARTIFACT`; `reconciled[].disposition` `settled_by_vendor_vote`; `reconciled[].settled_by`, `reconciled[].resolved_value`, `reconciled[].resolved_by`, `reconciled[].agreeing_vendors`; `summary.settled_by_vote_items`, `summary.settled_by_vote` | A vendor vote accepts a field the consensus left open and the record carries the agreed value, but the consensus finding on that field stays in the artifact the gate is handed, so the queue asked about a value the record had already accepted: on the commission run every one of the 532 fields the vote over the 24 OSALL pages settles was still queued. `--settled-by-vote` names the vote and the one input whose findings it answers; those findings move to `reconciled` with the vote and the value, never deleted. Another control's finding on the same field -- a validation or arithmetic finding about the agreed value -- is still asked. A settlement naming an artifact that is not an input, or a file that is not a `multi_engine_vote_v1` artifact, is refused. |
| An operator disposition retains what no control will answer | `client_review/queue.py` `operator_dispositions`, `consolidate(paths, accepted, governed, settled, disposed)`, `--dispositions ARTIFACT` on `final_review_queue.py` and `client_review_lane.py build`; artifact `operator_item_dispositions_v1` with `authorization_status: operator_authorized`, `authorization_id`, `dispositions[].review_item_id`, `dispositions[].rule`, `dispositions[].evidence`; `reconciled[].disposition` `dispositioned_by_operator`, `reconciled[].disposed_by`, `reconciled[].disposition_authorization_id`, `reconciled[].disposition_rule`, `reconciled[].disposition_evidence`; `summary.dispositioned_items`, `summary.disposition_artifacts`, `summary.dispositions_matching_no_item` | Some findings no control will ever answer -- a lane that returned no candidate, a provider's own review flag, a handwritten region under a comment-only policy -- and each holds its document out of canonical. An operator's decision retains each such item by its `review_item_id`, the digest of the finding itself, so a rebuilt queue cannot point the decision at another finding. Only an operator-authorized artifact is read. One naming no item, or an entry without a rule, is refused, and a disposition matching no item is counted rather than ignored. The item moves to `reconciled` with its rule and evidence, never deleted, and the review pack takes the same option, so both paths reconcile the same way. |
| The crediting rule credits a statement line by line | `attribution.py` `credit_by_line`, `LINE_CREDIT_CODE`, `--credit-by-line AUTHORIZATION`; register `reason_code` `credited_by_line`, `is_failure: false`, `line_credits[].line`, `.key`, `.key_field`, `.amount`, `crediting_authorization`; summary `credited_by_line_documents`, `credited_by_line_value`, `crediting_authorization` | A document whose lines name several jobs has an allocation question, not a missing attribution. Under an operator-authorized crediting rule, each line carrying money is credited to the key it prints. A line carries money when the first `document_value` line value field it carries is not zero, and a line printing two kinds of key is credited to the first in rank 1's order. Nothing is chosen among keys and no share is estimated: a document is credited only when every money line prints a key the corpus formats accept, and stays `unresolved` otherwise. A credited document carries no document-level key; the credits and the authorization ride on its register entry. |
| A re-read governs its own pages, and an answered register entry is not a question | `client_review/queue.py` `governed_pages`, `consolidate(paths, accepted, governed, settled)`, `--supersede ARTIFACT MANIFEST`; `reconciled[].disposition` one of `already_resolved`, `superseded_by_re_read`; `reconciled[].governed_by`; `summary.superseded_items`, `summary.superseded_by`; register entries with `is_failure: false`; `attribution.py` `DISPOSITION_CODES` `payment_record`, `blank_page` | A subset re-read replaces its pages' readings in the record, but the earlier readings' findings stay in the artifacts the gate is handed, so the queue asked about readings the record no longer held. `--supersede` names the earlier artifact and the re-read's manifest: its findings on those pages move to `reconciled` with the manifest that governs them, never deleted, and the re-read's own findings are supplied as inputs. A supersession naming an artifact that is not an input, or a manifest naming no page, is refused. An attribution register entry carrying a Phase 0 reason code (`is_failure: false`) is an answer; queued, it held its document out of canonical by its own answer. `payment_record` and `blank_page` are assigned only where the page image shows it: of 92 documents the client typed payments and 29 typed blank, the images bore out 45 and 5. |
...
| Client review pack reconciliation | `client_review_lane.py build` `--resolved-by`, `--supersede ARTIFACT MANIFEST`, `--settled-by-vote VOTE ARTIFACT`; `client_review/lane.py` `build_pack(..., resolved_by, supersede, settled_by_vote)`; `summary.reconciled_items`, `summary.resolution_artifacts`, `summary.superseded_items`, `summary.superseded_by`, `summary.settled_by_vote_items`, `summary.settled_by_vote`, `reconciled` | `final_review_queue.py` retains three kinds of finding rather than asking them: a document-type proposal classification already accepted with the same type, a finding on a page a subset re-read governs, and a finding on a field a vendor vote settled. The pack lane builds the same queue by a second path. With none of these options a pack issued from it re-asked 1,121 questions the run had answered; with `--resolved-by` alone, a pack from the commission run's final-queue inputs asked 36,796 items where the queue asks 35,742: 522 findings a re-read superseded and 532 a vote settled. It now takes all three, read by the queue's own `resolutions`, `governed_pages` and `vote_settlements`, so it refuses what the queue refuses before writing anything, and its summary counts what it retained as the queue's does, whether or not a pack is produced. Given the queue's inputs and options, the pack asks exactly the queue's items. A pack whose every finding was retained is refused with that count, not as a pack built from nothing. Two paths to one queue must reconcile the same way. |
