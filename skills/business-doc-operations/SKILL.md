...
| any | Client review pack | `client_review_lane.py build\|read-answers` | Queue, group, question, evidence and render in one command. Pass `--classifications` alongside `--consensus` for the same reason grouping needs it, and the gate's `--resolved-by`, `--supersede` and `--settled-by-vote` or the pack re-asks every finding the gate already reconciled, superseded or settled; pass `--scan-profile` or a blank page can become the worked example a client answers about; `read-answers` re-ingests the returned workbook. Threshold-gated per control. |
...
| — | Multi-engine vote | `multi_engine_vote.py` | Counts every reading the run retained, one vote per vendor, to settle fields consensus left open because it only ever compares two lanes. Run it before building the review queue: on one corpus it settled 6,951 fields for nothing, measured at 96.3% (two vendors) and 100% (three) against held-out truth. Over a subset re-read, pass `--pages` with that read's manifest, and give the queue `--settled-by-vote` with the vote and the re-read's consensus findings. |
